...
export const componentApiSchemas: Record<string, ComponentApiSchema> = {
  button: {
    slug: "button",
    source: "metadata",
    summary: "Action component for submit, navigation-like actions, destructive flows, loading states and icon buttons.",
    props: [
      { name: "variant", type: "'default' | 'secondary' | 'outline' | 'ghost' | 'destructive' | 'link'", defaultValue: "'default'", description: "Controls the visual hierarchy and action tone.", example: "variant=\"destructive\"" },
      { name: "size", type: "'sm' | 'default' | 'lg' | 'icon'", defaultValue: "'default'", description: "Controls padding, height and icon-only sizing.", example: "size=\"icon\"" },
      { name: "disabled", type: "boolean", defaultValue: "false", description: "Disables the action and removes it from normal interaction.", example: "disabled={isSaving}" },
    ...
    examples: [
      { title: "Basic usage", code: "<Button>Save changes</Button>" },
      { title: "Destructive action", code: "<Button variant=\"destructive\">Delete</Button>" },
    ],
    accessibility: ["Keep visible focus states.", "Use aria-label for icon-only buttons.", "Do not use disabled buttons for navigation links."],
  },
  input: {
    slug: "input",
    source: "metadata",
