    ...
    <div className="grid gap-4 xl:grid-cols-[0.24fr_1fr]">
      <div className="rounded-[22px] border border-zinc-200 bg-zinc-50 p-4">
        {["Overview", "Analytics", "Customers", "Orders", "Settings"].map((item, index) => (
          <div key={item} className={`mb-2 rounded-xl px-3 py-2.5 text-sm ${index === 0 ? "bg-zinc-950 text-white" : "bg-white text-zinc-600"}`}>
            {item}
          </div>
        ))}
      </div>
      <div className="space-y-4">
        <div className="flex flex-col gap-4 rounded-[22px] border border-zinc-200 bg-white p-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="text-sm text-zinc-500">Structured layout with reusable surface hierarchy.</p>
          </div>
          <div className="flex gap-3">
            <Input value={state.search} onChange={(event) => setState({ search: event.target.value })} placeholder="Search workspace..." className="w-48" />
        ...
        <div className="grid gap-4 md:grid-cols-[1fr_0.38fr]">
          <div className="h-48 rounded-[22px] border border-zinc-200 bg-[linear-gradient(180deg,rgba(59,130,246,0.12),transparent),radial-gradient(circle_at_bottom_left,rgba(34,197,94,0.18),transparent_40%)]" />
