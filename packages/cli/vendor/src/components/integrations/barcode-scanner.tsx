...
function BarcodeScannerComponent({
  ...
  const appendResult = React.useCallback((result: Result, source: "camera" | "image") => {
    const now = Date.now()
    if (source === "camera" && now - lastScanRef.current < Math.max(0, scanDelay)) return null
    if (source === "camera") lastScanRef.current = now
    ...
    return item
  }, [currentHistory, duplicateWindowMs, emitFeedback, maxHistory, onResult, scanDelay, selectedDeviceId, setCurrentHistory, setStatus])
